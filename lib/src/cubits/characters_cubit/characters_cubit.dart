...
class CharactersCubit extends Cubit<CharactersState> {
  ...
  try {
    ...
    if (cached != null && cached.isNotEmpty) {
      emit(state.copyWith(status: CharactersStatus.success, characters: cached));
      debugPrint("got from cache");
      return;
    }

    final apiCharacters = await _characterService.getAllCharacters(page: page);
    await _localDataSource.saveCharactersResponse(apiCharacters, page: page);
    debugPrint("got from api");

    emit(state.copyWith(status: CharactersStatus.success, characters: apiCharacters));
  } catch (e) {
    final cached = await _localDataSource.getAllCharacters(page: page);
    if (cached != null && cached.isNotEmpty) {
      emit(state.copyWith(status: CharactersStatus.success, characters: cached));
    } else {
      emit(state.copyWith(status: CharactersStatus.failure, errorMessage: e.toString()));
    }
  }
}
